<script lang="ts">
  ...
  let query = $state('');
  let selectedGroup = $state(formulaChapters[0].group);
  ...
  const mobileGroup = $derived(groups.some((entry) => entry.group === selectedGroup) ? selectedGroup : (groups[0]?.group ?? formulaGroups[0]));
  const mobileChapters = $derived(visible.filter((chapter) => chapter.group === mobileGroup));

  function openFormula(id: string): void {
    activeId = id;
    selectedGroup = formulaChapters.find((chapter) => chapter.id === id)?.group ?? selectedGroup;
    document.querySelector('.formula-library-reader')?.scrollTo({ top: 0, behavior: 'smooth' });
  }

  function openGroup(group: string): void {
    selectedGroup = group;
    const first = visible.find((chapter) => chapter.group === group);
    if (first) openFormula(first.id);
  }
</script>
...
<div class="modal-backdrop full-screen-on-mobile" role="presentation" onclick={(event) => event.target === event.currentTarget && onclose()}>
  <div class="formula-library-modal" role="dialog" aria-modal="true" aria-label="Atlas matemático del Modelo Estándar">
    ...
    <div class="formula-library-layout">
      <aside class="formula-library-index">
        <label><Search size={15}/><input bind:value={query} placeholder="Buscar QCD, Higgs, mezcla, Planck…"/></label>
        <nav class="formula-mobile-groups" aria-label="Áreas del atlas matemático">
        ...
        <small class="formula-index-label"><span class="desktop-index-copy">{visible.length} FÓRMULAS EN EL ÍNDICE</span><span class="mobile-index-copy">{visible.length} FÓRMULAS · DESLIZA HORIZONTALMENTE</span></small>
        <div class="formula-mobile-chapters" aria-label={`Fórmulas de ${mobileGroup}`}>
          {#each mobileChapters as chapter}
            <button class:active={active.id === chapter.id} type="button" aria-current={active.id === chapter.id ? 'page' : undefined} onclick={() => openFormula(chapter.id)}><span><b>{chapter.label}</b><small>{chapter.title}</small></span><ChevronRight size={14}/></button>
          {/each}
        </div>
        <nav class="formula-desktop-navigation">{#each groups as entry}<span>{entry.group}</span>{#each entry.chapters as chapter}<button class:active={active.id === chapter.id} type="button" onclick={() => openFormula(chapter.id)}><span><b>{chapter.label}</b><small>{chapter.title}</small></span><ChevronRight size={14}/></button>{/each}{/each}</nav>
        <footer>{visible.length} fórmulas en el índice</footer>
      </aside>
      <article class="formula-library-reader">
        ...
        <section class="formula-related"><small>CONEXIONES MATEMÁTICAS</small><h3>Continuar por</h3><div>{#each active.related as id}{#if formulaChapters.find((chapter) => chapter.id === id)}<button type="button" onclick={() => openFormula(id)}>{formulaChapters.find((chapter) => chapter.id === id)?.label}</button>{/if}{/each}</div></section>
